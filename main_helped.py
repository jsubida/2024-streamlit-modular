"""
Main file for the 'Tales from the Scale' project; only difference between 
this and `bonnie_prototype_main.py` is the filepath passed in to 
...
template_service.get_template("./resources/helped_template.json")

# Using streamlit secrets to set environment variables for langsmith/chain
os.environ["OPENAI_API_KEY"] = st.secrets['OPENAI_API_KEY']
os.environ["LANGCHAIN_API_KEY"] = st.secrets['LANGCHAIN_API_KEY']
os.environ["LANGCHAIN_PROJECT"] = template_service.project_name
...
if st.session_state['consent']:
    st.set_page_config(page_title="Tales From the Scale", page_icon="📖")
    st.title("Tales from the Scale ⚖️📖💪")
...
if st.session_state['llm_model'] == "gpt-4o":
    prompt_datacollection = template_service.prompt_datacollection
...
def getData(testing=False):
    ...
    Returns: 
    Nothing returned as all data is stored in streamlitchatmessages history msgs. 
    """
...
def extractChoices(msgs, testing):
    ...
    extraction_llm = ChatOpenAI(
        temperature=0.1, model=st.session_state.llm_model, openai_api_key=openai_api_key)

    # taking the prompt from lc_prompts.py file
    extraction_template = PromptTemplate(input_variables=[
                                         "conversation_history"], template=template_service.extraction_prompt)
    ...
    if testing:
        extractedChoices = extractionChain.invoke(
...
def collectFeedback(answer, column_id,  scenario):
    ...
    if score is not None:
        ...
        # and score value
        feedback_type_str = f"{answer['type']} {
            score} {answer['text']} \n {scenario}"
...
def summariseData(testing=False):
    ...
    prompt_template = PromptTemplate.from_template(
        template_service.prompt_one_shot)
    ...
    end_prompt = template_service.end_prompt_core
    ...
    for key in template_service.prompt_keys:
...
def scenario_selection(popover, button_num, scenario):
    ...
    with popover:
        ...
        st.markdown(f"How well does the scenario {
                    button_num} capture what you had in mind?")
        sliderOptions = ["Not really ", "Needs some edits",
                         "Pretty good but I'd like to tweak it", "Ready as is!"]
        slider_name = f'slider_{button_num}'

        st.select_slider("Judge_scenario", label_visibility='hidden', key=slider_name,
                         options=sliderOptions, on_change=sliderChange, args=(slider_name,))

        c1, c2 = st.columns(2)

        # the accept button should be disabled if no rating has been provided yet
        c1.button("Continue with this scenario 🎉", key=f'yeskey_{button_num}', on_click=click_selection_yes, args=(
            button_num, scenario), disabled=st.session_state['scenario_judged'])

        # the second one needs to be accessible all the time!
        c2.button("actually, let me try another one 🤨", key=f'nokey_{
                  button_num}', on_click=click_selection_no)
...
def reviewData(testing):
    ...
    if st.session_state['scenario_selection'] == '0':
        ...
        st.chat_message("ai").write(
            "Please have a look at the stories above. Use the 👍 and 👎  to leave a rating and short comment on each of the scenarios. Then pick the one that you like the most to continue. ")
        ...
        scenario_selection(
            p1, '1', st.session_state.response_1['output_scenario'])
        scenario_selection(
            p2, '2', st.session_state.response_2['output_scenario'])
        scenario_selection(
            p3, '3', st.session_state.response_3['output_scenario'])
    ...
    # if the user still wants to continue adapting
        ...
        with original:
            st.markdown(f"It seems that you selected a scenario that you liked: \n\n :green[{
                        package['scenario']}]")
            st.markdown(
                f"... but that you also think it: :red[{package['judgment']}]")
        ...
        with adapt_convo_container:
            ...
            if prompt:
                ...
                adaptation_prompt = PromptTemplate(
                    input_variables=["input", "scenario"], template=template_service.prompt_adaptation)
                json_parser = SimpleJsonOutputParser()
...
def stateAgent():
    ...
    if testing:
        print("Running stateAgent loop -- session state: ",
